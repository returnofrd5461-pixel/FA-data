...
- **이탈기간 칩도 동일 방식**: `LOST_PALETTE`(12색)를 기간 키(NN_NN) 시작월의 `MONTH_ORDER` 인덱스로 순환 할당(`lostChipStyle(k)`). 기존 10_11~06_07 색상 그대로 배치(회귀 방지), 신규 기간은 자동 발색 — 수동 맵(구 `LOST_CHIP`) 없음.
